...
\hline
Langfuse & Collects traces, latency, token usage, cost, and execution metrics. \\
\hline
...
The observability architecture of VIP is mainly based on Langfuse. Langfuse is used to collect and store traces, spans, latency information, token usage, model-related information, and error details from the runtime layer. Custom Python logs are also used for local debugging, while OpenTelemetry is used to propagate trace context between services.

\subsubsection{Langfuse-Based Observability Design}

During the first phase of the project, TraceAI was evaluated as a possible observability solution. However, this integration is not active in the current runtime. The platform later moved toward Langfuse as the main observability solution because it can be deployed locally as a self-hosted Docker stack and provides access to traces, spans, metrics, execution runs, and a dedicated dashboard.

In the current architecture, Langfuse
 is integrated mainly at the runtime layer. Generated agents initialize Langfuse when they start, and they create spans for request handling, agent execution, tool execution, and model calls. The orchestrator also initializes Langfuse and creates spans around workflow runs, workflow steps, and calls to generated agent endpoints.

The frontend backend does not emit Langfuse spans directly. Instead, it queries Langfuse data and exposes it to the frontend observability dashboard. This separation makes Langfuse the main observability backend, while the frontend acts mainly as a visualization layer.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/Langfuse-observability.png}
    \caption{Langfuse observability data flow in VIP}
    \label{fig:langfuse_observability_data_flow}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[tracelabel, below=0.45cm of response] {
            All spans are correlated in Langfuse using \texttt{vif.trace\_id}
        };
    \end{tikzpicture}
...
Each workflow execution generates a hierarchical trace composed of multiple spans, where each span corresponds to an individual agent invocation or tool execution. This hierarchical representation facilitates debugging, performance analysis, and root-cause identification.


\subsubsection{Observability Metrics (Langfuse)}

The VIP platform integrates Langfuse as its observability layer to monitor and analyze the execution of AI agents and multi-agent workflows. Unlike traditional software systems, agentic AI systems rely on probabilistic language models, which introduce non-deterministic behavior. As a result, classical logging alone is not sufficient to understand runtime execution. Observability is therefore required to ensure traceability, debugging capability, performance monitoring, and cost control.

Langfuse provides a unified observability model based on execution traces, spans, and runtime metrics. Each workflow execution generates a structured trace that captures the full lifecycle of an agent request, including intermediate steps, tool invocations, model calls, and final outputs. These traces allow fine-grained inspection of system behavior across distributed components.

\paragraph{Monitored Metrics}

The following metrics are collected during agent execution:

\begin{itemize}
    \item \textbf{Execution latency:} measures the response time of individual agents and complete workflows.
    \item \textbf{Token usage:} records input and output token consumption for each model invocation.
    \item \textbf{Estimated cost:} derived from token usage and model pricing information when available.
    \item \textbf{Execution traces:} provide an end-to-end representation of workflow execution.
    \item \textbf{Spans:} represent individual steps within an execution trace, such as agent calls or tool usage.
    \item \textbf{Tool invocations:} record external API or tool calls performed by agents.
    \item \textbf{Prompt version:} identifies the exact prompt template used during execution.
    \item \textbf{Model version:} specifies the language model and configuration used for inference.
    \item \textbf{Input/Output payloads:} store structured request and response data for debugging and reproducibility.
    \item \textbf{Execution status:} indicates whether an operation succeeded or failed.
    \item \textbf{Error information:} captures exceptions and failure reasons for diagnostic purposes.
\end{itemize}

\paragraph{Runtime Monitoring and Data Flow}

During execution, each generated agent emits observability events that are collected by Langfuse. These events are automatically associated with the corresponding execution trace and aggregated at workflow level by the orchestrator. This enables both local and global monitoring of system behavior.

The collected metrics are exposed through backend endpoints and visualized in the VIP dashboard. This allows users to analyze recent executions, inspect traces, monitor performance trends, and evaluate system reliability.

\paragraph{Logging vs Structured Observability}

In addition to Langfuse, the platform uses traditional runtime logging for local debugging. Generated agents and the orchestrator produce logs stored inside container environments and exposed through Docker stdout. These logs are primarily used for development and low-level inspection.

However, logs are not equivalent to structured observability data. While logs provide unstructured runtime information, Langfuse provides structured traces, spans, and metadata that allow systematic analysis of execution behavior. Both approaches are complementary: logging supports debugging during development, while Langfuse ensures production-grade observability.

\paragraph{Conclusion}

The integration of Langfuse significantly improves the operational control of the VIP platform. It enables full traceability of agent executions, supports performance and cost monitoring, and provides a structured foundation for debugging and optimization across distributed AI workflows.
...
The chapter then described the global architecture through the system context view, container view, and main platform components. These views showed how the frontend, backend, Agent Factory, generated agents, orchestrator, PostgreSQL, MLflow, langfuse, Docker environment, and LLM/SLM providers interact to support the lifecycle of configurable agents.
...
Finally, the observability architecture showed how Langfuse, trace identifiers, token usage, latency metrics, cost information, and runtime logs support operational monitoring across the orchestrator, generated agents, tools, and model calls.
